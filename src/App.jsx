...
const CROP_TIPS = [
  ...
  { crop:"Soybean", icon:"🫘", stage:"Flowering",      tip:"Avoid excess nitrogen at flowering. Ensure proper root nodules.",   alert:false },
];
...
export default function App() {
  ...
  // Log form state (lifted to App so sub-components don't remount on each render)
  ...
  const saveLog = () => {
    setLogs(prev => ({
      ...prev,
      [diaryDate]: { notes:logNotes, activities:logActivity, photos:logPhotos, alarms:logAlarms, updatedAt:new Date().toISOString() }
    }));
    setDiaryView("day");
  };
  ...
  // ════════════════════════════════════════════════════════════
  // SCREENS  — defined as plain functions (NO useMemo/useCallback wrapping)
  //            so React treats them as stable component types and never
  //            unmounts+remounts them mid-render (which was killing focus).
  // ════════════════════════════════════════════════════════════
          ...
          {TABS.map(({ id, Icon, label }) => {
            ...
            return (
              <button key={id} onClick={() => setTab(id)} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:3, background:"none", border:"none", cursor:"pointer", padding:"3px 8px", minWidth:50, position:"relative" }}>
                ...
                {badge>0 && (
                  <div style={{ position:"absolute", top:0, right:6, width:14, height:14, background:"#FF9800", borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center" }}>
