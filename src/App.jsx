...
export default function App() {
  ...
  // ── Farm Diary State ─────────────────────────────────────────
  // logs: { [dateKey]: { notes, activities:[{id,type,time,note}], photos:[dataURL], alarms:[{id,time,label,done}] } }
  const [logs, setLogs]             = useState(() => {
    try { return JSON.parse(localStorage.getItem("farmDiaryLogs") || "{}"); } catch { return {}; }
  });
  const [diaryView, setDiaryView]   = useState("calendar"); // "calendar" | "day" | "newLog" | "editLog"
  ...
  const [logActivity, setLogActivity]   = useState([]);   // [{type, time, note}]
  const [logPhotos, setLogPhotos]       = useState([]);   // [dataURL]
  const [logAlarms, setLogAlarms]       = useState([]);   // [{id, time, label, done}]
  ...
  useEffect(() => {
    localStorage.setItem("farmDiaryLogs", JSON.stringify(logs));
  }, [logs]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages(p => [...p, { sender:"ai", text:"⚠️ Unable to reach AI. Check your connection.", time:new Date() }]);
  ...
  const openNewLog = (dk) => {
    ...
    setLogActivity(existing?.activities || []);
    setLogPhotos(existing?.photos || []);
    setLogAlarms(existing?.alarms || []);
    setDiaryView("newLog");
  };
  ...
  const deleteLog = (dk) => {
    ...
    setLogs(prev => { const n = { ...prev }; delete n[dk]; return n; });
  ...
  const addAlarm = () => {
    ...
    if (Notification.permission === "granted") {
      const now = new Date();
      const alarmDate = new Date(diaryDate + "T" + newAlarm.time + ":00");
  ...
  const toggleAlarmDone = (id) => {
    ...
    // also update in saved logs
    setLogs(prev => {
      const day = prev[diaryDate];
      if (!day) return prev;
      return { ...prev, [diaryDate]: { ...day, alarms: day.alarms.map(a => a.id===id ? { ...a, done:!a.done } : a) } };
    });
  };
      ...
      <div style={{ padding:"18px 14px 24px" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:20 }}>
          {[
            ...
            { label:"Pending Alarms",val:Object.values(logs).reduce((a,l)=>a+(l.alarms||[]).filter(al=>!al.done).length,0), icon:"⏰" },
          ].map(({ label, val, icon }) => (
        ...
        <div style={{ background:"#fff", borderRadius:20, padding:"16px 14px", boxShadow:"0 4px 18px rgba(0,0,0,0.07)" }}>
            ...
            {cells.map((d, i) => {
              ...
              return (
                <button key={dk} onClick={() => { setDiaryDate(dk); setDiaryView("day"); }}
                  style={{ aspectRatio:"1", border:"none", borderRadius:10, cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:1, position:"relative",
          ...
          <div style={{ display:"flex", gap:14, marginTop:14, justifyContent:"center" }}>
            {[
              { color:C.sprout, label:"Has log" },
              { color:"#FF9800", label:"Pending alarm" },
              { color:"#2196F3", label:"Has photo" },
            ].map(({ color, label }) => (
  ...
  const DiaryDayView = () => {
    ...
    return (
      <div style={{ padding:"18px 14px 24px" }}>
        ...
        {/* Date header */}
        <div style={{ background:`linear-gradient(145deg, ${C.moss}, ${C.leaf})`, borderRadius:18, padding:"18px 18px", marginBottom:16 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <div>
              <p style={{ margin:0, fontSize:16, fontWeight:700, color:"#fff", fontFamily:"Georgia, serif" }}>
                {new Date(diaryDate+"T00:00:00").toLocaleDateString("en-IN",{weekday:"long"})}
              </p>
        ...
        {!log ? (
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            ...
            {log.notes && (
              <div style={{ background:"#fff", borderRadius:16, padding:16, boxShadow:"0 2px 10px rgba(0,0,0,0.06)" }}>
                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:10 }}>
                  <div style={{ width:30, height:30, background:"#FFF8E1", borderRadius:8, display:"flex", alignItems:"center", justifyContent:"center" }}>
            ...
            {log.activities?.length > 0 && (
              <div style={{ background:"#fff", borderRadius:16, padding:16, boxShadow:"0 2px 10px rgba(0,0,0,0.06)" }}>
                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12 }}>
                  <div style={{ width:30, height:30, background:"#F0FFF0", borderRadius:8, display:"flex", alignItems:"center", justifyContent:"center" }}>
                    <Sprout size={15} color={C.sprout}/>
                  </div>
            ...
            {log.photos?.length > 0 && (
              <div style={{ background:"#fff", borderRadius:16, padding:16, boxShadow:"0 2px 10px rgba(0,0,0,0.06)" }}>
                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12 }}>
                  <div style={{ width:30, height:30, background:"#E3F2FD", borderRadius:8, display:"flex", alignItems:"center", justifyContent:"center" }}>
                    <Camera size={15} color="#2196F3"/>
                  </div>
            ...
            {log.alarms?.length > 0 && (
              <div style={{ background:"#fff", borderRadius:16, padding:16, boxShadow:"0 2px 10px rgba(0,0,0,0.06)" }}>
                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12 }}>
                  <div style={{ width:30, height:30, background:"#FFF3E0", borderRadius:8, display:"flex", alignItems:"center", justifyContent:"center" }}>
                    <AlarmClock size={15} color={C.amber}/>
                  </div>
                ...
                {log.alarms.map((alarm, i) => (
                  <div key={alarm.id||i} style={{ display:"flex", gap:12, alignItems:"center", padding:"10px 0", borderBottom: i<log.alarms.length-1?"1px solid #F5F5F5":"none" }}>
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      <p style={{ margin:0, fontSize:11, color:"#aaa", display:"flex", alignItems:"center", gap:3 }}>
                        <Clock size={10}/> {alarm.time}
                      </p>
                    </div>
                    <span style={{ fontSize:10, fontWeight:700, padding:"3px 8px", borderRadius:20, background:alarm.done?"#E8F5E9":"#FFF3E0", color:alarm.done?C.sprout:C.amber }}>
                      {alarm.done ? "Done" : "Pending"}
                    </span>
                  </div>
                ))}
              </div>
    ...
    <div style={{ padding:"18px 14px 24px" }}>
      ...
      <div style={{ background:"#fff", borderRadius:16, padding:16, marginBottom:14, boxShadow:"0 2px 10px rgba(0,0,0,0.06)" }}>
        ...
        <textarea
          ...
          rows={4}
          style={{ width:"100%", border:"1.5px solid #EEE", borderRadius:10, padding:"10px 12px", outline:"none", fontSize:13, color:C.soil, resize:"none", fontFamily:"inherit", lineHeight:1.6, background:C.mist }}
        />
      </div>
      ...
      <div style={{ background:"#fff", borderRadius:16, padding:16, marginBottom:14, boxShadow:"0 2px 10px rgba(0,0,0,0.06)" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
          ...
          <button onClick={() => setAddActOpen(true)} style={{ background:C.fog, border:"none", borderRadius:8, padding:"6px 12px", cursor:"pointer", display:"flex", alignItems:"center", gap:4, fontSize:12, fontWeight:600, color:C.leaf }}>
            <Plus size={13}/> Add
          </button>
        </div>
      ...
      <div style={{ background:"#fff", borderRadius:16, padding:16, marginBottom:14, boxShadow:"0 2px 10px rgba(0,0,0,0.06)" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
          ...
          <button onClick={() => { requestNotifPerm(); setAddAlarmOpen(true); }} style={{ background:"#FFF3E0", border:"none", borderRadius:8, padding:"6px 12px", cursor:"pointer", display:"flex", alignItems:"center", gap:4, fontSize:12, fontWeight:600, color:C.amber }}>
            <Plus size={13}/> Add
          </button>
        </div>
  ...
  const DiaryScreen = () => {
    switch (diaryView) {
      case "day":     return <DiaryDayView />;
      case "newLog":  return <DiaryNewLog />;
      default:        return <DiaryCalendar />;
    }
  };
  ...
  const HomeScreen = () => (
    <div>
      <div style={{ background:`linear-gradient(145deg, ${C.moss} 0%, ${C.leaf} 60%, ${C.sprout} 100%)`, padding:"28px 20px 44px", position:"relative", overflow:"hidden" }}>
        <div style={{ position:"absolute", inset:0, opacity:0.04, backgroundImage:`url("data:image/svg+xml,%3Csvg width='40' height='40' viewBox='0 0 40 40' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='%23fff' fill-opacity='1' fill-rule='evenodd'%3E%3Cpath d='M0 40L40 0H20L0 20M40 40V20L20 40'/%3E%3C/g%3E%3C/svg%3E")` }} />
      ...
      <div style={{ background:"#fff", padding:"18px 14px 14px", boxShadow:"0 2px 8px rgba(0,0,0,0.05)" }}>
        ...
        <div style={{ display:"flex", gap:8 }}>
          <div style={{ flex:1, display:"flex", alignItems:"center", gap:8, background:C.fog, borderRadius:12, padding:"0 12px" }}>
            ...
            <input value={city} onChange={e=>setCity(e.target.value)} onKeyDown={e=>e.key==="Enter"&&fetchWeather(city)} placeholder="Search city or district…"
        ...
        <div style={{ padding:14 }}>
          ...
          <div style={{ background:"#F1F8E9", border:`1px solid ${C.lime}`, borderRadius:15, padding:14, marginBottom:14 }}>
            ...
            <ul style={{ margin:0, padding:"0 0 0 15px", color:C.bark, fontSize:12, lineHeight:1.8 }}>
              ...
              {weather.main.humidity<=75&&weather.main.temp<=35&&weather.wind.speed<=10&&weather.main.temp>=15&&<li>Conditions are favorable for most field operations today.</li>}
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"14px", display:"flex", flexDirection:"column", gap:12 }}>
        ...
        {messages.map((msg,i)=>(
          <div key={i} style={{ display:"flex", flexDirection:"column", alignItems:msg.sender==="user"?"flex-end":"flex-start" }}>
            ...
            <div style={{ maxWidth:"83%", padding:"11px 15px", borderRadius:msg.sender==="user"?"17px 17px 4px 17px":"4px 17px 17px 17px", background:msg.sender==="user"?`linear-gradient(135deg, ${C.leaf}, ${C.sprout})`:"#fff", color:msg.sender==="user"?"#fff":C.soil, boxShadow:"0 2px 8px rgba(0,0,0,0.07)", fontSize:13, lineHeight:1.65, whiteSpace:"pre-wrap" }}>{msg.text}</div>
      ...
      <div style={{ padding:"10px 13px 14px", background:"#fff", borderTop:"1px solid #F0F0F0" }}>
        <div style={{ display:"flex", gap:9, alignItems:"flex-end" }}>
          <div style={{ flex:1, background:C.fog, borderRadius:15, padding:"9px 13px" }}>
            <textarea rows={1} value={input} placeholder="Ask about crops, diseases, weather, schemes…"
      ...
      <div style={{ maxWidth:480, margin:"0 auto", minHeight:"100vh", background:C.mist, position:"relative" }}>
        ...
        <header style={{ background:`linear-gradient(135deg, ${C.soil}, ${C.bark})`, padding:"12px 18px", display:"flex", justifyContent:"space-between", alignItems:"center", position:"sticky", top:0, zIndex:200, boxShadow:"0 2px 14px rgba(0,0,0,0.25)" }}>
          ...
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            {/* Pending alarms badge */}
            {Object.values(logs).reduce((a,l)=>a+(l.alarms||[]).filter(al=>!al.done).length,0) > 0 && (
              <button onClick={()=>setTab("diary")} style={{ background:"#FF9800", border:"none", borderRadius:20, padding:"4px 10px", cursor:"pointer", display:"flex", alignItems:"center", gap:4 }}>
                ...
                <span style={{ fontSize:10, color:"#fff", fontWeight:700 }}>
                  {Object.values(logs).reduce((a,l)=>a+(l.alarms||[]).filter(al=>!al.done).length,0)}
                </span>
              </button>
            )}
        ...
        <nav style={{ position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:480, background:"#fff", borderTop:"1px solid #EFEFEF", padding:"7px 0 10px", display:"flex", justifyContent:"space-around", zIndex:200, boxShadow:"0 -3px 18px rgba(0,0,0,0.07)" }}>
          {TABS.map(({ id, Icon, label })=>{
            const active = tab===id;
            const pendingAlarms = id==="diary" ? Object.values(logs).reduce((a,l)=>a+(l.alarms||[]).filter(al=>!al.done).length,0) : 0;
            return (
              <button key={id} onClick={()=>setTab(id)} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:3, background:"none", border:"none", cursor:"pointer", padding:"3px 8px", minWidth:50, position:"relative" }}>
                ...
                {pendingAlarms>0&&<div style={{ position:"absolute", top:0, right:6, width:14, height:14, background:"#FF9800", borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center" }}>
                  <span style={{ fontSize:8, color:"#fff", fontWeight:700 }}>{pendingAlarms}</span>
                </div>}
